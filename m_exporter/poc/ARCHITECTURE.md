...
このドキュメントは `m_exporter/poc/` 配下のすべての PoC を、**「全体像 → データフロー → 各コンポーネント → 運用」** の順で読み解く資料です。
...
| ディレクトリ                | 役割                                                                                    | 既定ポート         |
| --------------------------- | --------------------------------------------------------------------------------------- | ------------------ |
| `go-exporter/`              | Go 製の Prometheus exporter。`gopsutil` で計測 → `/metrics`                              | `9100`             |
| `rust-exporter/`            | Rust 製の同等品。`sysinfo` で計測 → `/metrics`                                           | `9101`             |
...
| 9100   | go-exporter             | `poc/go-exporter/config.toml`               |
| 9101   | rust-exporter           | `poc/rust-exporter/config.toml`             |
...
sequenceDiagram
    ...
    P->>E: GET /metrics
    E-->>P: text exposition format<br/>m_exporter_cpu_usage_ratio{cpu="0"} 0.42
...
| 段              | 周期         | 設定箇所                                  |
| --------------- | ------------ | ----------------------------------------- |
| exporter 側収集 | 5s（既定）   | `poc/{go,rust}-exporter/config.toml` の `collector.interval` |
...
```json
{
  ...
  "data": {
    "resultType": "vector",
    "result": [
      { "metric": {"__name__":"up","job":"m_exporter_go"}, "value": [1777192400.123, "1"] }
    ]
  }
}
```
...
```sh
# 起動順（依存先から）
cd m_exporter/poc/go-exporter      && ./start.sh   # :9100
cd ../rust-exporter                && ./start.sh   # :9101
cd ../infra                        && ./init.sh    # 初回のみ
cd ../infra                        && ./start.sh   # :9090, :3000
...
poc/
├── go-exporter/
│   ├── m_exporter_go              ← go build 成果物 (gitignore)
│   └── .run/
│       ├── exporter.pid
│       └── exporter.log
├── rust-exporter/
...
| 変えたい項目                  | 編集対象                                                   |
| ----------------------------- | ---------------------------------------------------------- |
| exporter ポート               | `poc/{go,rust}-exporter/config.toml`                       |
...
| `m_exporter_collector_errors_total` が増えない    | Rust 側はそもそも未公開（sysinfo がエラー返さない） | Go 側でのみ参照                                                    |
...
```
poc/go-exporter/        ─ main.go, config.go, collector.go
poc/rust-exporter/src/  ─ main.rs, config.rs, collector.rs
