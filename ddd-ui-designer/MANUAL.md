...
画面パターンを自動導出する設計支援ツールの操作マニュアルです。
...
- **画面プレビュー** — 導出された画面（フォーム、テーブル、ウィザード等）をその場でモック表示。
...
- 表示モードは導出・生成・実行などの「結果」操作と相性が良いため、
  右ペインのタブ (🪟 / 🔀 / 📐) と組合せて使います。
...
| タブ | 内容 | 必要なもの |
|------|------|-----------|
| 🪟 **モックプレビュー** | 導出結果を画面ごとにモックUIで描画 | `▶ 画面を導出` 実行後 |
| 🔀 **画面遷移図** | Screen をノード、Transition を矢印にした SVG フロー図 | `▶ 画面を導出` 実行後 |
...
その後 `▶ 画面を導出` を押すと 4 つのパターンすべてのプレビューが見られます。
...
- フィールド数を変えて `▶ 画面を導出` を再実行 → どこで P1 → P2 → P4 に切り替わるか確認。
...
導出プレビューには各 Aggregate ごとにカードが並びます:
...
4. URL リンクをクリック → 導出したアプリが新しいタブで開く。
...
導出結果を「実際に動くプロトタイプ」として手元に持ち帰れます。

### 手順

1. ドメインを編集して `▶ 画面を導出` で結果を確認。
...
- **画面遷移** — 導出した `transitions` を state-machine として実装
