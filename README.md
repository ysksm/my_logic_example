...
| [chrome_dev_tool](./chrome_dev_tool) | ブラウザ計測 / 監視 | Go + React + CDP + Prometheus | 🟡 動作する試作 | — |
...
#### [chrome_dev_tool](./chrome_dev_tool) — Chrome DevTools 計測 + Prometheus Exporter
Chromium の起動/接続、CDP ログ収集、WebUI 観測に加え、`cdt-exporter` で Prometheus 向けにメトリクス公開できる Go 製ツール。`watch/list/snapshot` の CLI と React WebUI を同梱し、CPU 使用率 gauge などの DevTools 指標を取得可能。`make verify` で通常ビルドと `-tags wails` ビルドの整合性確認にも対応。
