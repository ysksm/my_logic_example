...
`poc/mac/go-exporter` (`:9100`) / `poc/mac/rust-exporter` (`:9101`) を **観測する側** の最小再実装。
...
cd exporters/poc/mac/go-exporter   && ./start.sh
...
curl -s 'http://localhost:9092/api/v1/query?query=mac_exporter_memory_used_ratio' | jq
curl -s 'http://localhost:9092/api/v1/query?query=sum(mac_exporter_cpu_usage_ratio)by(cpu)' | jq

# 4. 後始末
cd exporters/poc/mini-prometheus/go    && ./stop.sh
cd ../rust                             && ./stop.sh
```
...
3. **Save & test** が緑になれば OK。Explore で `mac_exporter_*` メトリクスをクエリ可能
