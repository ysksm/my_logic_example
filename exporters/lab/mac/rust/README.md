# lab/mac/rust — macOS パフォーマンスデータ取得 PoC（Rust 版）

Go 版（`lab/mac/go/`）と同じことを Rust でも試し、ライブラリの取り回しを比較するための作業ディレクトリです。
...
```sh
cd lab/mac/rust/<name>     # cpu / memory / disk / network / load
cargo run --release
```
